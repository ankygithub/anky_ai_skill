...
禁止以下 AI 腔调，改用自然、直接的技术写作风格：

❌ **禁止章节开头加"金句"**
- 不要写：`> "数据接入是实时数仓的地基。地基打不好，上面跑得越快，塌得越惨。"`
- 直接写：`本章聚焦 CDC。我们会从最基础的概念出发...`

❌ **禁止套路化过渡语**
- 不要写："学完本章，你应该能够..."、"接下来要解决一个核心问题"、"本章带你从零开始掌握"
- 直接写："本章讲解 CDC 的三种方案..."

❌ **禁止过度强调词**
- 减少使用："**核心**"、"**关键**"、"**重要**"、"**最**"
- 用具体内容代替：不说"这是最关键的一步"，说"这一步决定了数据一致性"

❌ **禁止教科书式总结**
- 不要写："这就是声明式开发的力量"、"这些问题的答案，决定了..."
- 直接陈述事实，让读者自己体会
...
本章聚焦 CDC。我们会从最基础的概念出发，讲清楚 CDC 的三种方案为什么最终只剩下了日志型 CDC 一条路；然后深入 Flink CDC 的核心——增量快照算法...

### 5.1 CDC 是什么，为什么需要它

CDC（Change Data Capture，变更数据捕获）是一种捕获数据库变更事件的技术。它关注的不是"数据库里现在有什么数据"，而是"数据库里发生了什么变化"...
```
...
HTML组件（callout/step/compare）内部**必须使用纯HTML语法**，禁止混用Markdown语法。

| 场景 | ❌ 错误写法 | ✅ 正确写法 |
|------|-----------|-----------|
| 加粗 | `**文字**` | `<strong>文字</strong>` |
| 列表 | `- 项目` | `<ul><li>项目</li></ul>` |
| 表格 | `\|列1\|列2\|` | `<table><thead>...</thead><tbody>...</tbody></table>` |
