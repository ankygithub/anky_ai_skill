...
| **写作粒度** | **Part级（多章/Agent）** | **章节级（1章/Agent）** |
...
| 模式 | 适用场景 | 流程 | 触发条件 |
|------|---------|------|---------|
| **模式A** | 从零开始 | 调研 → 生成Project → 写作 → 构建 | 用户只提供主题/想法 |
| **模式B** | 已有Project | 读取Project → 写作 → 构建 | 用户已提供计划文档/PROJECT.md |

> **【重要】模式判断原则**
> 
> 1. 如果用户**只提供主题**（如"写一本关于高可用的书"）→ 使用**模式A**
> 2. 如果用户**提供了计划文件**（如"读取 xxx.md 作为项目计划"）→ 使用**模式B**
> 3. 如果检测到 `{项目目录}/PROJECT.md` 已存在 → 询问用户是否使用已有Project
...
**3. 规划**
- 基于调研结果生成 `PROJECT.md`
- 包含：章节大纲表、调研资料索引、Agent并行分工方案、进度追踪表
- 与用户确认大纲后进入写作

---

### 模式B：基于已有Project（跳过调研）

**适用场景**：已用第三方AI（如GPT-4、Claude等）生成完整计划文档

**流程**：读取Project → 写作 → 构建

**特点**：
- ✅ **跳过调研阶段**：直接使用已有Project文件
- ✅ **跳过规划阶段**：不重新生成PROJECT.md
- ✅ **保留写作阶段**：按章节独立派发Agent
- ✅ **保留构建阶段**：正常生成所有产物
...
1. **用户提供计划文件路径**：
   ```
   用户：读取 D:\计划文件.md 作为项目计划
   → 使用模式B，直接开始写作阶段
   ```

2. **检测到已有PROJECT.md**：
   ```
   系统：检测到 PROJECT.md 已存在，是否使用已有计划文件？
   用户：是
   → 使用模式B，跳过调研和规划
   ```
...
3. **用户明确指示**：
   ```
   用户：Project已准备好，直接开始写作
   → 使用模式B
   ```

**注意事项**：
- 确保已有Project文件包含完整的章节大纲
- 建议Project文件格式与模板一致（包含章节、核心内容等）
- 如Project不完整，可能需要补充调研
...
> **【重要】写作粒度：章节级**
> 
> 无论计划文档如何分组（Part级或章节级），**每个章节必须独立派发Agent**。
> 
> 原因：
> - 避免模型输出限制导致内容压缩
> - 确保每章内容深度和完整性
...
> **写作铁律**：以下每个场景对应的【模板代码块】必须**原样复制使用**，禁止修改标签名！

---

**场景 1：章节核心要点、关键结论、重要总结**
...
**场景 2：安全警告、注意事项、禁忌、风险提示**

| 属性 | 值 |
|------|-----|
| 视觉样式 | 橙色警告块 |
| 触发条件 | 出现"危险/禁止/不要/风险/⚠️"等关键词时必须使用 |

**【模板-2】Callout 警告块（橙色）—— 直接复制以下内容：**
```markdown
...
|------|-----|
| 视觉样式 | 灰色信息块 |
| 触发条件 | 出现"参考/来源/延伸/详情"等关键词时建议使用 |

**【模板-3】Callout 信息块（灰色）—— 直接复制以下内容：**
```markdown
...
**场景 4：2步以上的操作步骤、配置流程、安装指南**

| 属性 | 值 |
|------|-----|
| 视觉样式 | 编号步骤卡片 |
| 强制要求 | 有序操作**必须使用**，禁止用普通 Markdown 列表替代 |

**【模板-4】步骤流程块 —— 直接复制以下内容（有几步就复制几组）：**
```markdown
<div class="step">
