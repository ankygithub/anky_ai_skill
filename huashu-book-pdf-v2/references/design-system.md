...
所有产物（单文件HTML、多文件阅读器）均支持字号切换功能：

- **小字号**（默认）：`--font-scale: 1`，正文约 10.5pt
- **中字号**：`--font-scale: 1.15`，正文约 12pt
- **大字号**：`--font-scale: 1.3`，正文约 13.5pt

**实现原理**：
- CSS 中所有 `font-size` 使用 `calc(原始值 * var(--font-scale))`
- 通过 JavaScript 修改 `:root` 上的 `--font-scale` 变量实现动态切换
- 用户偏好保存到 `localStorage`，刷新后自动恢复

**在HTML片段中编写时**：
- 直接使用 `px` 或 `pt` 单位即可，`build.js` 会自动通过 CSS 变量控制缩放
- 不要写死与字号相关的布局（如固定高度的容器），应使用 `em` 或 `rem`
