  ...
  for (const mdFile of mdFiles) {
    ...
    if (!hasFrontmatter) {
      ...
      if (!fmMatch) {
        ...
        // cover/backpage 不强制 title（与 check-md.js 规则保持一致）
        const typeVal = (fm.match(/type:\s*(\w+)/) || [])[1] || '';
        ...
        if (!fm.includes('title:') && !skipTitle) {
          errors.push({
            ...
            type: 'yaml',
            severity: 'error',
            message: '缺少 title 字段（cover/backpage 除外）',
            fixable: false
          });
        }
      }
    ...
    const fenceMaskGate = scanFenceMask(lines); // 共享围栏状态机
    let inPre = false;     // HTML <pre> 块状态

    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];

      if (fenceMaskGate[i]) continue; // 围栏行/围栏内行：# 注释不可误判为标题
    ...
    // 3. 禁用 HTML 标签检查（与 check-md.js TAG001 同步：组件已 Markdown 原生化）
    const forbiddenTags = [
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 跳过代码围栏：围栏内的 HTML 是字面量示例，不拦截
      if (/^(```|~~~)/.test(trimmedLine)) { inCodeFence = !inCodeFence; continue; }
      ...
      if (matches) {
        errors.push({
          ...
          type: 'tag',
          severity: 'error',
          message: `禁用 HTML 标签 ${[...new Set(matches.map(m => m.toLowerCase()))].slice(0, 3).join(' ')}。提示块用 > [!TIP]，步骤用 :::steps，对比用 :::compare（详见 research/components-quickref.md）`,
          fixable: false
        });
      }
    }
...
function writeErrorLog(errors, fixable) {
  ...
  if (fixable) {
    ...
    lines.push('2. 将 HTML 标签改为 Markdown 原生组件语法：');
    lines.push('   提示块 > [!TIP]/[!WARN]/[!NOTE]，步骤 :::steps，对比 :::compare');
    lines.push('3. 详见 research/components-quickref.md');
  }
...
if (!sourcePath) {
  ...
  // [0b] 组件语法门禁（check-md.js）：拦截禁用 HTML 标签、标题层级异常
  console.log('\n🔍 [0b] 组件语法门禁（check-md.js）...');
